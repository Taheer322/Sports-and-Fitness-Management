...
const App = () => {
  ...
  const LoginRegister = () => {
    ...
    const handleLoginSubmit = async (e) => {
      ...
      if (selectedRole === 'student') {
        ...
        // Coach login using contact number (not email)
        const coach = coaches.find(c => c.contact === loginEmail);
        if (coach) {
          handleLogin('coach', { ...coach, name: coach.c_name, coach_id: coach.coach_id });
        } else {
          alert('Coach not found! Please enter your contact number or contact administrator.');
        }
      }
    };
            ...
            <div className="login-box">
              ...
              {isLogin ? (
                <form onSubmit={handleLoginSubmit}>
                  ...
                  <div className="form-group">
                    <label>{selectedRole === 'coach' ? 'Contact Number' : 'Email'}</label>
                    <input
                      type={selectedRole === 'coach' ? 'text' : 'email'}
                      value={loginEmail}
                      onChange={(e) => setLoginEmail(e.target.value)}
                      placeholder={selectedRole === 'coach' ? 'Enter your contact number' : 'Enter your email'}
                      required
                    />
                  </div>
  ...
  const CoachDashboard = () => {
    ...
    const handleMarkAttendance = async (e) => {
      e.preventDefault();
      try {
        const currentCoach = currentUser?.coach_id 
          ? coaches.find(c => c.coach_id === currentUser.coach_id)
          : coaches[0];
        ...
        const response = await fetch(`${API_URL}/attendance`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...attendanceForm, coach_id: currentCoach.coach_id })
        });
        if (response.ok) {
    ...
    const coachEvents = events.filter(e => e.coach_id === currentCoach?.coach_id);
    const coachAttendance = attendance.filter(a => a.coach_id === currentCoach?.coach_id);

    // Get student progress data
    const getStudentProgress = (userId) => {
      const studentAttendance = attendance.filter(a => a.user_id === userId && a.coach_id === currentCoach?.coach_id);
    ...
    return (
      <div className="dashboard">
        <div className="dashboard-header">
          <h2>Coach Dashboard - {currentCoach?.c_name}</h2>
          <div className="nav-tabs">
            <button className={currentView === 'mark-attendance' ? 'active' : ''} onClick={() => setCurrentView('mark-attendance')}>Mark Attendance</button>
            <button className={currentView === 'schedule' ? 'active' : ''} onClick={() => setCurrentView('schedule')}>My Schedule</button>
            <button className={currentView === 'student-progress' ? 'active' : ''} onClick={() => setCurrentView('student-progress')}>Student Progress</button>
            <button className={currentView === 'my-events' ? 'active' : ''} onClick={() => setCurrentView('my-events')}>My Events</button>
            <button className={currentView === 'attendance-records' ? 'active' : ''} onClick={() => setCurrentView('attendance-records')}>Attendance Records</button>
          </div>
        </div>
            ...
            <form onSubmit={handleMarkAttendance} className="form">
              <div className="form-group">
                ...
                <select value={attendanceForm.user_id} onChange={(e) => setAttendanceForm({ ...attendanceForm, user_id: e.target.value })} required>
                  ...
                  {users.map(u => <option key={u.user_id} value={u.user_id}>{u.u_name} - {u.email}</option>)}
                </select>
              </div>
              <div className="form-row">
                <div className="form-group">
          ...
          <div className="section">
            ...
            <div className="coach-info-card">
              <div className="coach-detail">
                ...
                <span className="detail-value">{currentCoach?.c_name}</span>
              </div>
              <div className="coach-detail">
                <span className="detail-label">Specialization:</span>
                <span className="detail-value">{currentCoach?.specialization}</span>
              </div>
              <div className="coach-detail">
                <span className="detail-label">Schedule:</span>
                <span className="detail-value">{currentCoach?.schedule}</span>
              </div>
              <div className="coach-detail">
                <span className="detail-label">Contact:</span>
                <span className="detail-value">{currentCoach?.contact}</span>
              </div>
              <div className="coach-detail">
                <span className="detail-label">Total Events Assigned:</span>
                <span className="detail-value">{coachEvents.length}</span>
              </div>
              <div className="coach-detail">
                <span className="detail-label">Total Students Coached:</span>
                <span className="detail-value">{new Set(coachAttendance.map(a => a.user_id)).size}</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="section">
            ...
            {users.length === 0 ? (
              ...
              <table>
                ...
                <tbody>
                  {users.map(user => {
                    const progress = getStudentProgress(user.user_id);
                    const userEvents = participants.filter(p => p.user_id === user.user_id);
                    const avgScore = userEvents.length > 0 
                      ? (userEvents.reduce((sum, p) => sum + p.score, 0) / userEvents.length).toFixed(1)
                      : 'N/A';
                    ...
                    return (
                      <tr key={user.user_id}>
                        <td>{user.u_name}</td>
                        <td>{user.email}</td>
                        <td>{progress.totalSessions}</td>
                        <td>{progress.presentCount}</td>
                        <td>
                          <span className={`progress-badge ${
                            progress.attendanceRate >= 80 ? 'excellent' : 
                            progress.attendanceRate >= 60 ? 'good' : 
                            'needs-improvement'
                          }`}>
                            {progress.attendanceRate}%
                          </span>
                        </td>
                        <td>{avgScore !== 'N/A' ? `${avgScore} avg` : 'No events'}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {currentView === 'my-events' && (
          <div className="section">
            ...
            {coachEvents.length === 0 ? (
              <p className="no-data">No events assigned yet</p>
            ) : (
              <>
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="stat-icon">🏆</div>
                    <h4>{coachEvents.length}</h4>
                    <p>Total Events</p>
                  </div>
                  <div className="stat-card">
                    <div className="stat-icon">👥</div>
                    <h4>{participants.filter(p => coachEvents.some(e => e.event_id === p.event_id)).length}</h4>
                ...
                <table>
                  ...
                  <tbody>
                    {coachEvents.map(event => {
                      ...
                      const status = eventDate > today ? 'Upcoming' : eventDate.toDateString() === today.toDateString() ? 'Today' : 'Completed';
                      
                      return (
                        <tr key={event.event_id}>
                          <td><strong>{event.e_name}</strong></td>
                          <td>{event.sports}</td>
                          <td>{new Date(event.date).toLocaleDateString()}</td>
                          <td>{eventParticipants.length}</td>
                          <td>
            ...
            {coachAttendance.length === 0 ? (
              ...
              <>
                <div className="stats-grid">
                  <div className="stat-card">
                    <div className="stat-icon">📊</div>
                    <h4>{coachAttendance.length}</h4>
                    <p>Total Records</p>
                  </div>
                  <div className="stat-card">
                    <div className="stat-icon">✅</div>
                    <h4>{coachAttendance.filter(a => a.status === 'present').length}</h4>
                    <p>Present</p>
                  </div>
                  <div className="stat-card">
                    <div className="stat-icon">❌</div>
                    <h4>{coachAttendance.filter(a => a.status === 'absent').length}</h4>
                    <p>Absent</p>
                  </div>
                </div>
                <table>
                  ...
                  <tbody>
                    {coachAttendance.sort((a, b) => new Date(b.date) - new Date(a.date)).map(att => {
                      ...
                      return (
                        <tr key={att.attendance_id}>
                          ...
                          <td><span className={`status ${att.status}`}>{att.status}</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
